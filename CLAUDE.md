...
| Frontend (React + Vite) | Done | All 5 pages implemented with routing |
| CSV parser (smart column detection) | Done | Two-pass matching, BOM handling, real FileMaker exports |
| Pipeline (geocode, imagery, detection) | Done (code) | Needs API key in `.env` to test end-to-end |
| SSE streaming pipeline | Done | Real-time progress events in Dashboard |
| Communication tracking | Scaffolded | Backend API ready, no frontend integration yet |

### What's next

1. Add Google Maps API key, test pipeline on real property data
2. Keyboard shortcuts in ReviewQueue (arrow keys, number keys for findings)
3. Pipeline progress indicator improvements
4. Communication tracking UI (Phase 2)
