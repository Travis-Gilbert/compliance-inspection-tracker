"""
Priority scoring, buyer summaries, and Haversine clustering.

Carried over verbatim from FastAPI backend. Pure Python computation on
dicts. No framework dependencies.
"""
import math
from datetime import date, datetime
from typing import Optional

from tracker.schemas import RESOLVED_FINDINGS
...
def compute_priority_score(prop: dict, as_of: Optional[date] = None) -> tuple[float, dict]:
    ...
    elif finding in RESOLVED_FINDINGS:
        components["finding"] = -18.0
    else:
    ...
    for buyer in buyer_map.values():
        if buyer["property_count"]:
            buyer["average_priority_score"] = round(
                buyer["average_priority_score"] / buyer["property_count"], 2,
            )
...
def haversine_clusters(
    ...
    for prop in geocoded:
        ...
        clusters.append({
            ...
            "average_priority_score": round(
                sum(float(p.get("priority_score", 0.0)) for p in cluster_props) / len(cluster_props), 2,
            ),
            "properties": sorted(
                [
                    {
                        "id": p["id"], "address": p.get("address", ""),
    ...
    clusters.sort(key=lambda item: (-item["property_count"], -item["average_priority_score"], item["cluster_id"]))
    return clusters
