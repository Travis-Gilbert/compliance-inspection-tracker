...
Carried over verbatim from FastAPI backend. Pure dict-to-CSV generation.
...
def export_properties_csv(properties: list[dict], include_detection: bool = True) -> str:
    ...
    headers = [
        "Address", "Parcel ID", "Buyer Name", "Email", "Organization",
        "Purchase Type", "Program", "Closing Date", "Commitment",
        "Compliance 1st Attempt", "Compliance 2nd Attempt",
        "Finding", "Notes", "Reviewed Date",
    ]
    if include_detection:
        headers.extend([
            "Detection Label", "Detection Score",
            "Street View Available", "Street View Date",
            "Street View Historical Date", "Street View Historical Path",
        ])
...
def export_inspection_list_csv(properties: list[dict]) -> str:
    """Export properties flagged for physical inspection."""
    output = io.StringIO()
    writer = csv.writer(output)

    writer.writerow([
        "Address", "Parcel ID", "Buyer Name", "Program",
        "Detection Notes", "Staff Notes", "Inspection Result", "Inspection Date",
    ])
