...
import { importCSV, importCSVText } from "@/lib/api";
...
export default function ImportPage() {
  ...
  const setSelectedFiles = (selectedFiles: FileList | null) => {
    const nextFiles = Array.from(selectedFiles || []).filter(Boolean);
    setFiles(nextFiles);
    if (nextFiles.length > 0) setImportText("");
  };

  const handleImport = async () => {
    const trimmedText = importText.trim();
    if (files.length === 0 && !trimmedText) return;
    ...
    try {
      ...
      if (files.length > 0) {
        response = {
          batch_id: "", total_rows: 0, imported: 0, inserted: 0,
          updated: 0, skipped: 0, errors: [] as string[], filesProcessed: files.length,
        };

        for (let index = 0; index < files.length; index += 1) {
          const file = files[index];
          setImportProgress({ current: index + 1, total: files.length, name: file.name });
          ...
          try {
            ...
            response.errors.push(
              ...(fileResult.errors || []).map((entry: string) => `${file.name}: ${entry}`)
            );
          } catch (fileError: any) {
            response.skipped += 1;
  ...
  return (
    <div className="max-w-3xl space-y-6">
      ...
      {error && <InlineNotice tone="error" title="Import failed" message={error} />}
          ...
          {files.length > 0 ? (
            <div>
              ...
              <div className="mt-2 flex flex-wrap justify-center gap-2 text-xs text-gray-500">
                ...
                {files.length > 6 && (
                  <span className="rounded bg-gray-100 px-2 py-1">+{files.length - 6} more</span>
                )}
              </div>
            </div>
          ) : (
      ...
      {result?.errors?.length > 0 && (
        <div className="rounded-lg border border-orange-200 bg-orange-50 p-4">
          ...
          <div className="mt-2 space-y-1 text-xs text-orange-700">
            {result.errors.map((entry: string, index: number) => (
