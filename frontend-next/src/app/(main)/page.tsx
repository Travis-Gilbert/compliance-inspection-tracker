...
function PipelineProgress({ events }: { events: any[] }) {
  if (!events.length) {
    return null;
  }
...
export default function Dashboard() {
  ...
  const [pipelineNotice, setPipelineNotice] = useState<any>(null);
  ...
  return (
    <div className="space-y-6">
      ...
      {stats.by_compliance_status && Object.keys(stats.by_compliance_status).length > 0 && (
        <div className="rounded-lg border border-gray-200 bg-white p-5">
          <h3 className="mb-3 font-heading font-semibold text-gray-900">Compliance Status</h3>
            ...
            {Object.entries(stats.by_detection).map(([label, count]) => {
              ...
              return (
                <div key={label} className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-gray-700">{count as number} properties</span>
                </div>
              );
            })}
          </div>
            ...
            {Object.entries(stats.by_finding).map(([finding, count]) => {
              ...
              return (
                <div key={finding} className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-gray-700">{count as number}</span>
                </div>
              );
            })}
          </div>
