...
function ReviewQueueInner() {
  ...
  const [filter, setFilter] = useState(
    () => searchParams.get("filter") || "unreviewed",
  );
  ...
  const [detectionFilter, setDetectionFilter] = useState(
    () => searchParams.get("detection") || "all",
  );
  const [complianceFilter, setComplianceFilter] = useState(
    () => searchParams.get("compliance") || "all",
  );
  const [taxFilter, setTaxFilter] = useState(
    () => searchParams.get("tax") || "all",
  );
  ...
  const loadProperties = useCallback(async () => {
    ...
    try {
      ...
      if (complianceFilter !== "all") {
        params.compliance_status = complianceFilter;
      }
      if (taxFilter !== "all") {
        params.tax_status = taxFilter;
      }
  ...
  useEffect(() => {
    ...
    if (complianceFilter !== "all") {
      nextParams.set("compliance", complianceFilter);
    }
    if (taxFilter !== "all") {
      nextParams.set("tax", taxFilter);
    }
    if (page > 0) {
  ...
  useEffect(() => {
    ...
    setFocusedIndex(-1);
    setExpandedId(null);
  }, [filter, sort, search, programFilter, detectionFilter, complianceFilter, taxFilter]);
  ...
  const handleFindingUpdate = async (id: number, finding: string) => {
    setNotice(null);
    try {
  ...
  const selectAll = () =>
    setSelected(new Set(properties.map((property: any) => property.id)));
  const selectNone = () => setSelected(new Set());

  const handleBatchUpdate = async (finding: string) => {
    const label =
      FINDINGS.find((item) => item.value === finding)?.label || finding;
    ...
    try {
      ...
      setNotice({
        ...
        message:
          error.message || "The selected properties could not be updated.",
      });
    }
  };
  ...
  useEffect(() => {
    const handler = (event: KeyboardEvent) => {
      ...
      if (
        ...
        event.preventDefault();
        router.push(`/property/${properties[focusedIndex].id}`);
      } else if (event.key === " " && focusedIndex >= 0) {
        event.preventDefault();
        setExpandedId((prev) =>
          prev === properties[focusedIndex].id
            ? null
            : properties[focusedIndex].id,
        );
      } else if (
        event.key >= "1" &&
        event.key <= "6" &&
  ...
  }, [focusedIndex, properties, router]);
  ...
  const laneCounts: Record<string, number> = {
    ...
    likely_demolished:
      stats?.unreviewed_by_detection?.likely_demolished || 0,
  ...
  const applyLane = (lane: (typeof REVIEW_FAST_LANES)[number]) => {
    setFilter(lane.params.filter || "unreviewed");
    setSort(lane.params.sort || "priority");
    setProgramFilter((lane.params as any).program || "all");
    setDetectionFilter((lane.params as any).detection || "all");
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">
          Review unreviewed properties first, then narrow by program or
          detection signal when you need to move quickly.
        </p>
      </div>
      ...
      <div className="grid gap-3 rounded-lg border border-gray-200 bg-white p-4 md:grid-cols-[auto_auto_auto_1fr_auto]">
        ...
        {/* Search + detection + compliance + tax */}
      ...
      {stats && (
        <div className="flex flex-wrap items-center gap-4 text-xs text-gray-500">
          ...
          <span className="text-orange-600">
            {stats.needs_inspection} need inspection
          </span>
      ...
      {loading ? (
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-8 text-center">
          ...
          <p className="mt-2 text-xs text-gray-400">
            Try clearing a filter, or import more properties from the Import
            page.
          </p>
        </div>
      ) : (
        <div className="space-y-2">
          {properties.map((property: any, index: number) => {
            const finding = FINDINGS.find(
              (item) => item.value === property.finding,
            );
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-2 flex flex-wrap gap-1.5">
                        ...
                        {property.compliance_status &&
                          property.compliance_status !== "unknown" && (
                            <ComplianceStatusBadge
                              status={property.compliance_status}
                            />
                          )}

                        {/* Tax status indicator */}
                        {property.tax_status &&
                          property.tax_status !== "unknown" && (
                            <span
                              className={`rounded px-1.5 py-0.5 text-xs font-medium ${
                                property.tax_status === "delinquent"
                                  ? "bg-red-50 text-red-700"
                                  : property.tax_status === "payment_plan"
                            ...
                            >
                              Tax:{" "}
                              {TAX_STATUSES.find(
                                (s) => s.value === property.tax_status,
                              )?.label || property.tax_status}
                            </span>
                          )}
                        ...
                        {/* Detection badge */}
                        {detection &&
                          property.detection_label !== "unprocessed" && (
                            <span
                      ...
                      <div className="flex-1 space-y-3">
                        {detection &&
                          property.detection_label !== "unprocessed" && (
                            ...
                            >
                              Detection: {detection.label} (score:{" "}
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {FINDINGS.map((findingOption, findingIndex) => (
                            <button
                              ...
                              style={{
                                color:
                                  property.finding === findingOption.value
                                    ? "#fff"
                                    : findingOption.color,
                                background:
                                  property.finding === findingOption.value
          ...
          <div className="flex shrink-0 gap-1.5">
            {FINDINGS.map((findingOption) => (
              <button
                ...
                style={{
                  color: findingOption.color,
                  background: findingOption.bg,
                }}
              >
                {findingOption.label}
              </button>
